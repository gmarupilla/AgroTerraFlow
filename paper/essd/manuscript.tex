...
\Author[1]{Gnaneswara}{Marupilla}
\Author[2]{Chandhini}{Bayina}

\affil[1]{Independent Researcher \& Software Engineer (Scientific Computing)}
\affil[2]{University of Central Missouri, Warrensburg, Missouri, United States}
...
\label{sec:labels}
Labels derive from the USDA RMA \emph{Cause of Loss} Summary-of-Business files (public, pipe-delimited, 1989--present). For each county-crop-year we compute:
\begin{itemize}
...
\begin{itemize}
\item \textbf{Temporal} --- train on crop-years $\leq 2015$; test on held-out years \textbf{2012, 2017, 2022, 2023} (including the 2012 extreme drought and the recent 2022/2023 seasons). This is the primary, operationally realistic split.
...
\begin{table}[t]
\caption{Corn, temporal split (train $\leq 2015$, test $=$ 2012/2017/2022/2023). Headline metrics from the v0.2 reference build.}
\begin{tabular}{lll}
\tophline
Task & Model & Headline \\
\middlehline
...
\codedataavailability{The benchmark table, official splits, build manifest, and baseline leaderboard are archived on Zenodo at https://doi.org/10.5281/zenodo.21208651 (concept DOI, resolving to the latest version) under CC-BY-4.0. The assembly and evaluation pipeline is the open-source (MIT) \texttt{terraflow.drought} sub-package of TerraFlow (https://github.com/gmarupilla/AgroTerraFlow); the exact release used to build this dataset is tagged and archived alongside the data. Upstream inputs are public: USDA RMA Cause of Loss and Summary-of-Business files, USDA-NASS QuickStats, and the flashdry predictor corpus (derived from MODIS NDVI, ERA5-Land, Daymet, and the US Drought Monitor).}
