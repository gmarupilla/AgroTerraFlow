...
### Added
- **Drought-Impact Prediction Benchmark (v0) — new self-contained sub-package `benchmarks/drought_impact/`.** A prediction-ready benchmark whose target is *insured drought loss* (USDA RMA Cause-of-Loss indemnity), not drought severity (USDM) or yield — the gap no existing drought benchmark fills. Scope: corn, 6-state Corn Belt (IL/IN/IA/MN/MO/NE), 2000–2023; unit of observation = (`GEOID`, corn crop-year). Two tasks: loss-cost **regression** (`drought_loss_cost` = drought indemnity / county liability) and **binary** significant-loss classification. The package has its own `pyproject.toml` and does **not** depend on `terraflow` (spin-out ready); it vendors the deterministic build-fingerprint primitives from `terraflow/core/run_identity.py` so every build writes a reproducible `manifest.json`. Modules: `rma` (30-field COL parse + download), `labels`, `predictors` (aggregate flashdry `_anom` features over a growing-season window @`cutoff_doy` for early-warning framing), `coverage` (`insured_acre_fraction` bias column), `assemble`, `splits` (temporal / spatial-block / LOYO — spatial mirrors `terraflow/validation.py::_assign_block_ids`), `baselines` (naive / severity-index / ML), `metrics`, plus an argparse `cli` and thin `scripts/`. Ships a Gebru et al. `datasheet.md`, a benchmark-card `README.md`, a docs page (`docs/benchmarks/drought-impact.md`), and a self-contained demo notebook (`docs/notebooks/08_drought_impact_benchmark.ipynb`). All external inputs (flashdry paths, RMA source URL) are config values, so the pipeline runs on synthetic fixtures in CI and on real data locally; 36 unit + end-to-end tests validate the wiring. The benchmark leaderboard is populated by a local run against real RMA + flashdry data (marked *pending* in the README). Root `pyproject.toml` gains `testpaths`/`norecursedirs` so the core `pytest` run stays scoped to `tests/` and the sub-package is tested independently (its code is excluded from the 85% coverage floor).
